...
class TestDatastoreManager(unittest.TestCase):
    ...
    def test_open_datastore_from_info(self):
        # invoke code
        dsinfo = datastore._make_dsinfo({'dsid': 'xyzzy', 'handle': 'deadbeef', 'rev': 0})
        ds = self.manager.open_datastore_from_info(dsinfo)
...
class TestDatastore(unittest.TestCase):
    ...
    def test_load_snapshot(self):
        ...
        self.assertEqual(t.get('r1'), datastore.Record(t, 'r1', {'foo': 42}))
    ...
    def test_await_deltas(self):
        ...
        self.assertEqual(out,
                         {'t1': [datastore.Record(t1, 'r1', {'foo': 0}),
                                 datastore.Record(t1, 'r2', None),
                                 ],
                          })
        self.assertEqual(self.datastore.list_table_ids(), set(['t1']))
        self.assertEqual(t1.get('r1'), datastore.Record(t1, 'r1', {'foo': 0}))
...
class TestTable(unittest.TestCase):
    ...
    def test_get__get_or_insert(self):
        ...
        expected = datastore.Record(self.table, 'r1', {'foo': 42})
        self.assertEqual(r1, expected)
        self.assertTrue(r1 is r2)
        self.assertTrue(r2 is r3)
    ...
    def test_insert(self):
        ...
        self.assertTrue(self.table.get(r1.get_id()) is r1)
        self.assertTrue(self.table.get(r2.get_id()) is r2)
...
class TestRecord(unittest.TestCase):
    ...
    def test_eq_ne(self):
        # Have to use an internal way to create another record with the same key.
        record2 = datastore.Record(self.table, 'r1', {'foo': 42})
        record3 = datastore.Record(self.table, 'r1', {'foo': 42, 'bar': 0})
        record4 = self.table.get_or_insert('r3', foo=42)  # Different record ID.
        record5 = self.datastore.get_table('t2').get_or_insert('r1', foo=42)  # Different Table.
        self.assertTrue(self.record == record2)
        self.assertFalse(self.record == record3)
        self.assertFalse(self.record == record4)
        self.assertFalse(self.record == record5)
        self.assertFalse(self.record == 'blah')
        self.assertFalse(self.record != record2)
        self.assertTrue(self.record != record3)
        self.assertTrue(self.record != record4)
        self.assertTrue(self.record != record5)
...
class TestList(unittest.TestCase):
    ...
    def test_list_wrappers(self):
        ...
        self.assertTrue(foo.get_record() is r1)
